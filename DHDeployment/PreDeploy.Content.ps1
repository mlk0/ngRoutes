...
try {
	...
	log-info "($OctopusEnvironmentName) Content deployment for $OctopusProjectName $OctopusReleaseNumber"
...
try {
	...
	log-info "($OctopusEnvironmentName) Executing content deployment script DeployContent.ps1"
	push-location "$TargetPath"
	$DeployEsaDbScript= $TargetPath + "\DeployContent.ps1"
	log-info "DeployContent : " $DeployEsaDbScript
	. $DeployEsaDbScript | out-file "$TargetPath\$dblog" -append

	CheckLogFile "$TargetPath\$dblog" $db_error_list
	pop-location
	log-info "($OctopusEnvironmentName) Content deployment completed."
	...
	if ($OctoSMTPServer) {
		...
		$Subject = "$RFCNum $OctopusEnvironmentName" + ": $OctopusProjectName $OctopusReleaseNumber Content package deployment Completed"
