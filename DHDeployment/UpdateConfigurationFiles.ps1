...
function tokenupdate([string]$file, [string]$filename) {
	...
	Select-String -path $file -pattern $regex -AllMatches | Foreach-object {
		for ($i=0; $i -lt $_.matches.count;$i++) {
			$word = $($_.matches[$i]) -replace "\[\$","" -replace "\$\]",""
           ...
           # log-info "This is is the TokenList: $TokenList"
			if ( $TokenList -notmatch "$word" ) {
				...
				if ($tmp) {
                   ...
                   # log-info "This is tmp: $tmp"
                    
                    if ("$word" -eq "SKIPJACKPREFIX") {
                        $tmp = $tmp -replace '[.]',''
                    }
                    
                    [string]$split_1, [string]$split_2 = $tmp -split "\|"
                    #log-info "split_1 = $split_1"
                    #log-info "split_2 = $split_2"
                    $split_1 = $split_1 -replace '^(.*?)__', "[$"
                    $split_1 = $split_1 -replace "__", "$]"
                    $tmp = $split_1 + "|" + $split_2
